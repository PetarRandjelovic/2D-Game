...
void rafgl_raster_draw_spritesheet_my_method(rafgl_raster_t *raster, rafgl_spritesheet_t *spritesheet, int sheet_x, int sheet_y, int x, int y)
{
    int fl, fr, fu, fd;
    int flc, frc, fuc, fdc;
    int xi, yi;
    ...
    for(yi = fuc; yi < fdc; yi++)
    {
        for(xi = flc; xi < frc; xi++)
        {
            ...
            if(sampled.rgba != RAFGL_COLOUR_KEY.rgba)
            {
                if(sampled.rgba==(255,255,255))
                 pixel_at_pm(raster, xi, yi) = sampled;
            //    pixel_at_pm(raster, xi, yi) = sampled;


                sampled.r= sampled.r *1.2;
                sampled.g=sampled.g*1.2;
                sampled.b=sampled.b*1.2;
               pixel_at_pm(raster, xi, yi) = sampled;
